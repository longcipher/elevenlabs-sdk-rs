...
**MANDATORY**: Use `soldeer` for installing dependencies with `forge`. **STRICTLY FORBIDDEN** to use git submodules for dependency management.

1.  **Install Dependencies**: Always use `forge soldeer install` to install Solidity libraries.
2.  **Configuration**: Ensure `foundry.toml` includes `[soldeer]` section for dependency configuration.
3.  **Avoid Git Submodules**: Never initialize or use `git submodule add` for dependencies. Use soldeer remappings instead.

**Example `foundry.toml` config:**
```toml
[soldeer]
...
-   **Update Dependencies**: `forge soldeer update`
-   **Build**: `forge build`
-   **Test**: `forge test`

### Best Practices
-   Pin dependency versions to ensure reproducible builds.
-   Use soldeer remappings for clean import paths.
...
config = "0.15.19"
shadow-rs = "1.5.0"
ecdysis = "1.0.1" # For graceful restarts in binary crates
...
7.  **Graceful Restarts (Binary/Executable Crates)**:
    -   **STRICTLY** use `ecdysis` for graceful restarts in binary crates (e.g., servers or daemons).
    -   Ensure the main loop or server process integrates with `ecdysis` signal handling.
...
-   [ ] **HTTP**: Uses `hpx` with `rustls`.-   [ ] **Graceful Restarts**: Uses `ecdysis` in binary crates.-   [ ] **Database**: Uses `sqlx::query_as` + `FromRow`.
